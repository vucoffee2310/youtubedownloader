...
import { checkOverflow, toPx, LRUCache } from "../utils.js";
...
export class FontSizeCalculator {
  ...
  #calculateSize(textElement, container, { width, height }) {
    ...
    const fontSize = this.#fitTextToContainer(textElement, container, width, height);
  ...
  #calculateInitialGuess(width, height, textLength) {
    ...
    const guess = Math.sqrt(area / textLength) * CONFIG.OVERLAY.FONT_SIZE_GUESS_MULTIPLIER;
    return Math.max(
      CONFIG.OVERLAY.MIN_FONT_SIZE,
  ...
  #tryUpscale(textElement, container, currentSize) {
    ...
    if (checkOverflow(container, CONFIG.OVERLAY.OVERFLOW_TOLERANCE)) {
      return this.#binarySearchFontSize(textElement, container, currentSize, upscaledSize);
    }
