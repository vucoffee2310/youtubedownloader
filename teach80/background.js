importScripts("shared-config.js", "shared-utils.js");

const state = {
  dashboardTabId: null,
  automationTabIds: new Set(),
};

const listeners = {
  dashboardCleanup: null,
  automationCleanups: new Map(),
};

const dashboard = {
  open() {
    if (state.dashboardTabId) {
      UTILS.tab.exists(state.dashboardTabId).then(exists => {
        exists ? UTILS.tab.focus(state.dashboardTabId) : this.create();
      });
    } else {
      this.create();
    }
  },
  create() {
    UTILS.tab.create(chrome.runtime.getURL(SHARED.URL.dashboard)).then(tab => {
      if (listeners.dashboardCleanup) {
        chrome.tabs.onRemoved.removeListener(listeners.dashboardCleanup);
      }
      state.dashboardTabId = tab.id;
      listeners.dashboardCleanup = tabId => {
        if (tabId === state.dashboardTabId) {
          state.dashboardTabId = null;
          chrome.tabs.onRemoved.removeListener(listeners.dashboardCleanup);
          listeners.dashboardCleanup = null;
        }
      };
      chrome.tabs.onRemoved.addListener(listeners.dashboardCleanup);
    });
  },
};

const automation = {
  async open(model = SHARED.DEFAULTS.model, tabType = SHARED.TAB_TYPE.REGULAR) {
    const url = `${SHARED.URL.aiStudio}?model=${model}`;
    const tab = await UTILS.tab.create(url, false);
    state.automationTabIds.add(tab.id);
    setTimeout(() => UTILS.tab.focus(state.dashboardTabId), SHARED.TIMING.focusDelay);
    relay({ type: SHARED.MSG.TAB_CREATED, tabId: tab.id, tabType });

    const onLoad = (tabId, info) => {
      if (tabId === tab.id && info.status === "complete") {
        chrome.tabs.onUpdated.removeListener(onLoad);
        UTILS.tab.exists(tab.id).then(exists => {
          if (exists) {
            setTimeout(() => this.inject(tab.id), SHARED.TIMING.injectionDelay);
          } else {
            listeners.automationCleanups.delete(tab.id);
          }
        });
      }
    };
    chrome.tabs.onUpdated.addListener(onLoad);
    listeners.automationCleanups.set(tab.id, onLoad);
    return tab.id;
  },

  async inject(tabId) {
    try {
      if (!(await UTILS.tab.exists(tabId))) return;
      await chrome.scripting.executeScript({
        target: { tabId },
        files: ["shared-config.js", "shared-utils.js", "content-core.js", "content-init.js"],
      });
    } catch (err) {
      UTILS.msg.toTab(state.dashboardTabId, {
        type: SHARED.MSG.TAB_ERROR,
        tabId,
        error: `Injection failed: ${err.message}`,
      });
    }
  },
};

const relay = msg => UTILS.msg.toTab(state.dashboardTabId, msg);

const relayTypes = new Set([
  SHARED.MSG.LOG,
  SHARED.MSG.AUDIO_STATUS,
  SHARED.MSG.AUTOMATION_STATUS,
  SHARED.MSG.AUTO_DELETE_STATUS,
]);

const handlers = {
  [SHARED.MSG.OPEN_AI_STUDIO]: async msg => {
    try {
      const tabType = msg.tabType || SHARED.TAB_TYPE.REGULAR;
      const tabId = await automation.open(msg.model || SHARED.DEFAULTS.model, tabType);
      return { success: !!tabId, tabId, tabType };
    } catch (err) {
      return { success: false, error: err.message };
    }
  },
  [SHARED.MSG.CONTENT_READY]: (msg, sender) => {
    const tabId = sender.tab?.id;
    if (UTILS.validate.tabId(tabId)) {
      state.automationTabIds.add(tabId);
      relay({ ...msg, tabId });
    }
    return { success: true };
  },
  [SHARED.MSG.XHR_EVENT]: (msg, sender) => {
    relay({ ...msg, tabId: sender.tab?.id });
    return { success: true };
  },
  [SHARED.MSG.KEEP_ALIVE]: () => ({ success: true }),
};

chrome.action.onClicked.addListener(() => dashboard.open());

chrome.runtime.onMessage.addListener((msg, sender, respond) => {
  if (relayTypes.has(msg.type)) {
    relay(msg);
    respond({ success: true });
    return true;
  }
  const handler = handlers[msg.type];
  if (!handler) {
    respond({ success: false, error: "Unknown" });
    return true;
  }
  Promise.resolve(handler(msg, sender)).then(respond);
  return true;
});

chrome.tabs.onRemoved.addListener(tabId => {
  state.automationTabIds.delete(tabId);
  const listener = listeners.automationCleanups.get(tabId);
  if (listener) {
    chrome.tabs.onUpdated.removeListener(listener);
    listeners.automationCleanups.delete(tabId);
  }
  relay({ type: SHARED.MSG.TAB_REMOVED, tabId });
});

setInterval(() => {}, SHARED.TIMING.keepAliveInterval);